...
`--max` caps detail pages per site-and-model pair, not per run, so a bare walk across two sites and four models can still open up to 10 detail pages on each of the eight pairs. That cap counts matching candidates, not raw page visits: a detail page the walk opens and then rejects for being some other model (a plain Corolla on a Corolla Hybrid walk, say) never spends any of the cap, so the walk keeps taking links until the cap is met with real matches or the search page runs out of links to try. A pair's own coverage is only stamped on the run once that pair's walk actually finishes; a pair that errors (a page that never loads, a site that fails outright) is reported inline and the walk moves on to the next pair rather than aborting the whole run, and a walk interrupted partway through (Ctrl-C, a crash) leaves the pairs that already finished marked covered and the rest untouched. At the end, a summary table lists, per site and model, how many detail pages were visited, how many candidates were upserted, and how many were dropped for having no VIN, followed by the usual new/price-dropped/gone diff for the whole run.

Model filtering is per site, since the two sites expose it differently. Cars.com separates a hybrid or plug-in variant from its base model in its own search filter (`models[]=toyota-corolla_hybrid` is a different facet from `models[]=toyota-corolla`, and the same goes for Camry Hybrid vs. Camry and Prius Prime vs. Prius), so the walk's search URL asks for that exact facet value and the first page of results is already just that model. Carvana has no confirmed facet that does the same, so the walk still searches its base model and leans on the cap-doesn't-count-mismatches behavior above: it opens up to three times `--max` candidate links from the search page, rejecting any that turn out to be some other model, until it has filled the cap with real matches or run out of candidates to try.
