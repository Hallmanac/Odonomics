...
A walked page counts as a hybrid model's match when the extraction names the hybrid, or when the page's own title line reads "<year> <make> <base model> Hybrid" (for example "2023 Toyota Corolla Hybrid") even though the extraction split it into a base model and a trim. Only that title line decides it: a plain Corolla page that merely mentions a Corolla Hybrid elsewhere is still dropped as the wrong model, and the saved candidate carries the walked model either way.
