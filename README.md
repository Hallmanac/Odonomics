...
odo walk [cars.com|carvana|autotrader] [--model "Make Model"] [--max N]
...
- `odo rank --detail` adds one line under each ranked and over-budget row, giving that vehicle's loan payment beside its during-loan total (`loan payment $324-$348  of during $592-$627`). It is a separate line rather than a column, so the row above it keeps its 80-column fit.
