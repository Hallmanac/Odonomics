...
The card is where the figures are read, because a card and a detail page do not show the same price. A card shows what the buyer pays delivered, and a detail page shows the car's price at its lot. On a store-transfer Insight the card read $27,697 with "Price includes $699 shipping", and its detail page read $26,998. So the posting's price is the card's, and a link whose card shows no price is dropped as missing a price and is never stored at its lot price. A known car's card, touched without a detail visit, moves the stored price the same way.
...
- A known car's card replaces the stored posture and shipping fee without a detail visit, and a card that says nothing leaves them alone.
