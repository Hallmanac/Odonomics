...
The price comes from the extraction model, so it is not always the headline price: in run 20260926-121057 one Autotrader page (VIN 4T1DAACK2SU548011) stored its $26,695 listing price and not its $28,193 total. When the page prints both a listing price and a total that differ by exactly its fee lines, the walk compares the stored price with them. A stored price equal to the listing price means the fees are not in it, so an all-in reading becomes itemized and the fees are added. A stored price equal to the total means they are in it, so an itemized reading becomes all-in and the fees are not counted twice. Any other stored price leaves the reading as it was.
