...
A bare `odo walk` walks the whole shortlist in one command: cars.com first, then Carvana, then Autotrader, then CarMax. On each site it visits every model in the scenario's `allowedModels`, in order, and it prints which site and model it's about to visit before each one. `odo walk cars.com`, `odo walk carvana`, `odo walk autotrader`, or `odo walk carmax` narrows it to that one site, still with every model. `--model "Make Model"` narrows either form to that one model exactly, matching the scenario's own casing case-insensitively.
